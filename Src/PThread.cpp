...
Mutex::Mutex() : UniqueValue{PTHREAD_MUTEX_INITIALIZER} {}

Mutex::~Mutex() {
  int ret = pthread_mutex_destroy(&Get());
  if (ret != 0) STDERR_WARN_CUSTOM_ERRNO("pthread_mutex_destroy()", ret);
}

void Mutex::Lock() {
  int ret = pthread_mutex_lock(&Get());
  if (ret != 0) THROW_CUSTOM_ERRNO("pthread_mutex_lock()", ret);
}

void Mutex::Unlock() {
  int ret = pthread_mutex_unlock(&Get());
  if (ret != 0) THROW_CUSTOM_ERRNO("pthread_mutex_unlock()", ret);
}

/* ----- Cond ----- */

Cond::Cond() : UniqueValue{PTHREAD_COND_INITIALIZER} {}

Cond::~Cond() {
  int ret = pthread_cond_destroy(&Get());
  if (ret != 0) STDERR_WARN_CUSTOM_ERRNO("pthread_cond_destroy()", ret);
}

void Cond::Wait(Mutex& mutex) {
  int ret = pthread_cond_wait(&Get(), &mutex.Get());
  if (ret != 0) THROW_CUSTOM_ERRNO("pthread_cond_wait()", ret);
}

void Cond::TimedWait(Mutex& mutex, const timespec& ts) {
  int ret = pthread_cond_timedwait(&Get(), &mutex.Get(), &ts);
  if (ret != 0 && ret != ETIMEDOUT)
    THROW_CUSTOM_ERRNO("pthread_cond_timedwait()", ret);
}

void Cond::Signal() {
  int ret = pthread_cond_signal(&Get());
  if (ret != 0) THROW_CUSTOM_ERRNO("pthread_cond_signal()", ret);
}
...
Thread::Thread(Routine target, void* args) : IsJoined{false} {
  ...
  if (ret != 0) THROW_CUSTOM_ERRNO("pthread_create()", ret);
}
...
void Thread::Join() {
  ...
  if (ret != 0) THROW_CUSTOM_ERRNO("pthread_join()", ret);
...
void Thread::Kill(int sig) {
  ...
  if (ret != 0) THROW_CUSTOM_ERRNO("pthread_kill()", ret);
}
